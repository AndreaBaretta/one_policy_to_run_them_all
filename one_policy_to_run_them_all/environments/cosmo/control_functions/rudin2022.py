...
class Rudin2022Control:
    ...
    def process_action(self, action):
        ...
        torques = self.seen_p_gain * self.p_gain_noise_factor * (target_joint_positions - self.env.data.qpos[self.env.joint_mask_qpos] + self.joint_position_offset) \
                  - self.seen_d_gain * self.d_gain_noise_factor * self.env.data.qvel[self.env.joint_mask_qvel]
