...
class RudinOwnVarReward:
    def __init__(self, env,
                 curriculum_steps=50e6,
                 tracking_xy_velocity_command_coeff=3.0, tracking_yaw_velocity_command_coeff=1.5,
                 ...
                 joint_velocity_coeff=0.0, joint_acceleration_coeff=2.5e-7, joint_torque_coeff=2e-5, action_rate_coeff=1e-2,
                 collision_coeff=1e0, base_height_coeff=3e1, nominal_trunk_z=1.007, air_time_coeff=1e-1, air_time_max=0.5, symmetry_air_coeff=0.5):
        self.env = env
        ...
        self.time_since_last_touchdown_left = 0
        self.time_since_last_touchdown_right = 0
    ...
    def step(self, action):
        self.time_since_last_touchdown_left = 0 if self.env.check_collision("floor", "foot_l_collision") else self.time_since_last_touchdown_left + self.env.dt
        self.time_since_last_touchdown_right = 0 if self.env.check_collision("floor", "foot_r_collision") else self.time_since_last_touchdown_right + self.env.dt
        self._prev_joint_vel = np.array(self.env.data.qvel[self.env.joint_mask_qvel])
    ...
    def reward_and_info(self, info, done):
        ...
        joint_positions = np.array(self.env.data.qpos[self.env.joint_mask_qpos])
        lower_limit_penalty = -np.minimum(joint_positions - self.joint_limits[self.env.joint_mask_jnt_vals-1, 0], 0.0).sum()
        upper_limit_penalty = np.maximum(joint_positions - self.joint_limits[self.env.joint_mask_jnt_vals-1, 1], 0.0).sum()
        ...
        velocity_norm = np.sum(np.square(np.array(self.env.data.qvel[self.env.joint_mask_qvel])))
        velocity_reward = curriculum_coeff * self.joint_velocity_coeff * -velocity_norm

        # Joint acceleration reward
        acceleration_norm = np.sum(np.square(self._prev_joint_vel - np.array(self.env.data.qvel[self.env.joint_mask_qvel]) / self.env.dt))
        acceleration_reward = curriculum_coeff * self.joint_acceleration_coeff * -acceleration_norm

        # Joint torque reward
        torque_norm = np.sum(np.square(np.array(self.env.data.qfrc_actuator[self.env.joint_mask_qvel])))
        ...
        # Air time reward
        foot_right_on_ground = self.env.check_collision("floor", "foot_r_collision")
        foot_left_on_ground = self.env.check_collision("floor", "foot_l_collision")
        air_time_reward = 0.0
        if foot_right_on_ground == 1:
            air_time_reward += self.time_since_last_touchdown_right - self.air_time_max
        if foot_left_on_ground:
            air_time_reward += self.time_since_last_touchdown_left - self.air_time_max
        air_time_reward = curriculum_coeff * self.air_time_coeff * air_time_reward

        # Symmetry reward
        symmetry_air_violations = 0.0
        if not foot_left_on_ground and not foot_right_on_ground:
        ...
        # power = np.sum(abs(self.env.current_torques) * abs(self.env.data.qvel[self.env.joint_mask_qvel]))
